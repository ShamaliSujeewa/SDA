...
shinyServer(function(input,output,session){
  ...
  #demodata <- read.csv("completedata.csv")
  ...
  output$dROCCurve<-renderPlot({db3()
    ddf3<-demodata
    fulmod <- glm(CKD~Duration+Age+Gender+Occupation+Liquor+Water_Source+Water_Storage+Chronic_Illness, data=ddf3 ,family = binomial(link="logit"))
    #prob=predict(step(fulmod),type=c("response"))
    #pred <- prediction(prob, ddf3[,input$dres] )
    #roc = performance(pred, "tpr", "fpr")
    #windowsFonts(A = windowsFont("Times New Roman"))
    #plot(roc,  lwd=2, family="A", cex.lab=2, cex.axis=2)
    #lines(x=c(0, 1), y=c(0, 1), col="grey", lty=2)
    
    glm_link_scores <- predict(step(fulmod), ddf3, type="link")
    
    glm_response_scores <- predict(step(fulmod), ddf3, type="response")
    
    score_data <- data.frame(link=glm_link_scores, 
                             response=glm_response_scores,
                             bad_widget=ddf3[,input$dres],
                             stringsAsFactors=FALSE)
    
    
    plot(roc(ddf3[,input$dres], glm_response_scores, direction="<"),
  ...
  output$ROCCurve<-renderPlot({b3()
    df3<-data3()
    fulmod <- glm(input$binmod, data=df3 ,family = binomial(link="logit"))
    
    glm_link_scores <- predict(step(fulmod), df3, type="link")
    
    glm_response_scores <- predict(step(fulmod), df3, type="response")
    
    score_data <- data.frame(link=glm_link_scores, 
                             response=glm_response_scores,
                             bad_widget=df3[,input$res],
                             stringsAsFactors=FALSE)
    
    
    plot(roc(df3[,input$res], glm_response_scores, direction="<"),
         col="red", lwd=3, main="ROC Curve")
